...
import { getAllCategoryForAdmin, updateCategoryAdmin } from '../../redux/Action/category';

const AdminDashboardUpdateCategory = () => {
   ...
   const {error, message, categoryloading} = useSelector((state) => state.category);
    ...
    const submitHandler = async (e) => {
        e.preventDefault();
     await dispatch(updateCategoryAdmin(oldName, newName,image));
  ...
  return (
    ...
    <form action="" className=' w-full flex flex-col justify-center items-center p-3' onSubmit={submitHandler}>
        <div className=" 800px:w-[500px] w-full flex flex-col mt-8 gap-4 p-1">
            <label htmlFor="name" className='pl-3 text-white text-[17px] font-[200]'>Old Category Name</label>
            <input type="text"
               id='name'
        ...
        <div className=" 800px:w-[500px] w-full flex flex-col mt-8 gap-4 p-1">
            <label htmlFor="name" className='pl-3 text-white text-[17px] font-[200]'>New Category Name</label>
            <input type="text"
               id='name'
...
export default AdminDashboardUpdateCategory